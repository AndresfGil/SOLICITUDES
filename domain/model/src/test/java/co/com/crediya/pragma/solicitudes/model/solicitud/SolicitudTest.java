package co.com.crediya.pragma.solicitudes.model.solicitud;

import org.junit.jupiter.api.Test;

import java.math.BigDecimal;

import static org.junit.jupiter.api.Assertions.*;

class SolicitudTest {

    @Test
    void testSolicitudBuilder() {
        Solicitud solicitud = Solicitud.builder()
                .idSolicitud(1L)
                .monto(new BigDecimal("5000000"))
                .plazo(24)
                .email("test@example.com")
                .documentoIdentidad("12345678")
                .idEstado(1L)
                .idTipoPrestamo(1L)
                .build();

        assertEquals(1L, solicitud.getIdSolicitud());
        assertEquals(new BigDecimal("5000000"), solicitud.getMonto());
        assertEquals(24, solicitud.getPlazo());
        assertEquals("test@example.com", solicitud.getEmail());
        assertEquals("12345678", solicitud.getDocumentoIdentidad());
        assertEquals(1L, solicitud.getIdEstado());
        assertEquals(1L, solicitud.getIdTipoPrestamo());
    }

    @Test
    void testSolicitudNoArgsConstructor() {
        Solicitud solicitud = new Solicitud();
        
        assertNull(solicitud.getIdSolicitud());
        assertNull(solicitud.getMonto());
        assertNull(solicitud.getPlazo());
        assertNull(solicitud.getEmail());
        assertNull(solicitud.getDocumentoIdentidad());
        assertNull(solicitud.getIdEstado());
        assertNull(solicitud.getIdTipoPrestamo());
    }

    @Test
    void testSolicitudAllArgsConstructor() {
        Solicitud solicitud = new Solicitud(
                1L,
                new BigDecimal("3000000"),
                12,
                "user@example.com",
                "87654321",
                2L,
                2L
        );

        assertEquals(1L, solicitud.getIdSolicitud());
        assertEquals(new BigDecimal("3000000"), solicitud.getMonto());
        assertEquals(12, solicitud.getPlazo());
        assertEquals("user@example.com", solicitud.getEmail());
        assertEquals("87654321", solicitud.getDocumentoIdentidad());
        assertEquals(2L, solicitud.getIdEstado());
        assertEquals(2L, solicitud.getIdTipoPrestamo());
    }

    @Test
    void testSolicitudSettersAndGetters() {
        Solicitud solicitud = new Solicitud();
        
        solicitud.setIdSolicitud(5L);
        solicitud.setMonto(new BigDecimal("7000000"));
        solicitud.setPlazo(36);
        solicitud.setEmail("newuser@example.com");
        solicitud.setDocumentoIdentidad("11111111");
        solicitud.setIdEstado(3L);
        solicitud.setIdTipoPrestamo(3L);

        assertEquals(5L, solicitud.getIdSolicitud());
        assertEquals(new BigDecimal("7000000"), solicitud.getMonto());
        assertEquals(36, solicitud.getPlazo());
        assertEquals("newuser@example.com", solicitud.getEmail());
        assertEquals("11111111", solicitud.getDocumentoIdentidad());
        assertEquals(3L, solicitud.getIdEstado());
        assertEquals(3L, solicitud.getIdTipoPrestamo());
    }

    @Test
    void testSolicitudToBuilder() {
        Solicitud original = Solicitud.builder()
                .idSolicitud(1L)
                .monto(new BigDecimal("5000000"))
                .plazo(24)
                .email("test@example.com")
                .documentoIdentidad("12345678")
                .idEstado(1L)
                .idTipoPrestamo(1L)
                .build();

        Solicitud modified = original.toBuilder()
                .monto(new BigDecimal("6000000"))
                .plazo(30)
                .build();

        assertEquals(1L, modified.getIdSolicitud());
        assertEquals(new BigDecimal("6000000"), modified.getMonto());
        assertEquals(30, modified.getPlazo());
        assertEquals("test@example.com", modified.getEmail());
        assertEquals("12345678", modified.getDocumentoIdentidad());
        assertEquals(1L, modified.getIdEstado());
        assertEquals(1L, modified.getIdTipoPrestamo());
    }
}
