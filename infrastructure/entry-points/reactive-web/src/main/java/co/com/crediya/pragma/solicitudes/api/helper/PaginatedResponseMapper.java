...
public class PaginatedResponseMapper {
    ...
    public Mono<PaginatedResponseDTO<Solicitud>> toPaginatedResponse(
            SolicitudRepository.PaginatedResult<Solicitud> result) {
        
        return result.content().collectList()
            .map(content -> PaginatedResponseDTO.<Solicitud>builder()
                .content(content)
